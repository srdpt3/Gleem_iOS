...
struct HistoryView: View {
    ...
    init(){
        self.historyViewModel.loadHistoricData()
    }
    var body: some View {
        VStack(spacing: 0){
            HStack{
            ...
            GeometryReader{mainView in
                ...
                ScrollView{
                    if(!self.historyViewModel.historicData.isEmpty){
                        VStack(spacing: 15){
                            
                            // setting name as id...
                            
                            ForEach(self.historyViewModel.historicData , id: \.self) {data in
                                
                                // Album View....
                                
                                GeometryReader{item in
                                    
                                    HistoricVote(data: data)
...
struct HistoricVote : View {
    ...
    var body: some View{
        ...
        HStack{
            ...
            AnimatedImage(url: URL(string:data.imageLocation))
                .resizable()
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                HStack(alignment: .center, spacing: 2) {
                    Image(systemName: "person.2").foregroundColor(APP_THEME_COLOR).padding(.trailing, 5)
                    Text(VOTENUM + " " + String(data.numVote))
                        .font(Font.custom(FONT, size: 13)).foregroundColor(APP_THEME_COLOR)
                }
                HStack(alignment: .center, spacing: 2) {
                    Image(systemName: "clock").foregroundColor(APP_THEME_COLOR).padding(.trailing, 5)
                    Text(VOTE_TIMESTAMP + timeAgoSinceDate(Date(timeIntervalSince1970: data.createdDate ), currentDate: Date(), numericDates: true)).font(Font.custom(FONT, size: 13)).foregroundColor(APP_THEME_COLOR)
                }
                
                
                HStack(alignment: .center, spacing: 2) {
                    Text(data.attrNames[0] + ": " + String(data.attr1) + ", ").font(Font.custom(FONT, size: 11)).foregroundColor(APP_THEME_COLOR)
                    Text(data.attrNames[1] + ": " + String(data.attr2) + ", ").font(Font.custom(FONT, size: 11)).foregroundColor(APP_THEME_COLOR)
                    
                    Text(data.attrNames[2] + ": " + String(data.attr3)).font(Font.custom(FONT, size: 11)).foregroundColor(APP_THEME_COLOR)
                }
                HStack(alignment: .center, spacing: 2) {
                    Text(data.attrNames[3] + ": " + String(data.attr4) + ", " ).font(Font.custom(FONT, size: 11)).foregroundColor(APP_THEME_COLOR)
                    Text(data.attrNames[4] + ": " + String(data.attr5)).font(Font.custom(FONT, size: 11)).foregroundColor(APP_THEME_COLOR)
